 ...
 *   1.5.2 分岐待ち（waitForMore）が既存の設問数だけで成功扱いになる欠陥を修正。
 *         テンプレート取得失敗時の無言フォールバックを修正。base64 変換をチャンク化。
 *         UI・コメント・ドキュメントから絵文字と装飾記号を排除し、表記を整理。
 *         役目を終えた「4番テスト」ツール一式と一時ファイルを撤去（引用符の要否は決着済み）。
 */
const VERSION = '1.5.2';
...
function main() {
  ...
  console.log('  ' + path.join(DIST, `選択肢の採り直し_${probeVersion}.txt`) + '  (ブックマークレット ' + kb(probeOpenUrl.length) + ')');
